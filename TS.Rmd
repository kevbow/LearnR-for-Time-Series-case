...
Bagaimana performa model yang telah dibuat? Coba cek keakuratan hasil model dengan metrics MAPE. Silakan gunakan fungsi `accuracy()` pada kedua hasil forecast di atas.
...
question_parsons(
  initial = c(
    "autolayer(des_forecast$fitted, series = 'train')",
    "autoplot()",
    "lstock_ts", 
...
best_arima_forecast <- forecast(elec_ar2, h=8*12)
```
...
Carilah model hasil ARIMA yang memiliki akurasi paling baik. Model ini yang nantinya akan kita gunakan pada proses selanjutnya.
...
### 6. Melakukan prediksi pada data test
Berdasarkan model-model yang telah dibuat di atas, gunakan fungsi `forecast()` untuk melakukan prediksi pada data test yang telah dipisahkan. Jangan lupa untuk menggunakan jumlah frequency yang sesuai dengan jumlah data test yang kita tentukan. Gunakan 2 model yaitu model Triple Exponential Smoothing dengan nama `tes_forecast` dan model hasil SARIMA terbaik yang ditemukan dengan nama `best_arima_forecast`. Setelah prediksi dibuat, coba lihat hasilnya menggunakan visualisasi. Gunakan fungsi `autoplot()` dan `autolayer()` untuk menampilkan hasil prediksi dari kedua forecast yang telah dilakukan.
...
### 7. Evaluasi hasil model 
Bagaimana performa model yang telah dibuat? Coba cek keakuratan hasil model dengan metrics MAPE. Gunakan fungsi `MAPE()` pada bagian ini.
...
quiz(
  question("Setelah melihat hasil evaluasi di atas, model mana yang lebih baik akurat?",
           type = "single", correct = "Yap, tepat! Model ARIMA memiliki nilai MAPE yang lebih kecil, error yang dihasilkan lebih kecil sehingga dapat dikatakan model ARIIMA lebih baik.",
           incorrect = "Jawaban Anda masih kurang tepat", 
           allow_retry = TRUE, random_answer_order = FALSE,
           answer("Model dengan metode Triple Exponential Smoothing"),
           answer("Model dengan metode ARIMA", correct = TRUE))
)
```
